...
export const FAQ = ({ className }: FAQProps) => {
  ...
  const sortedData = useMemo(() => {
    if (activeCategory === 'Все' && searchQuery === '') {
      ...
      return questions.filter((question) => question.title.toLowerCase().includes(searchQuery.toLowerCase()));
    } else {
      return questions.filter(
        (question) =>
          question.categories.includes(activeCategory!) &&
      ...
      <div className={cls.block}>
        <div className={cls.top}>
          ...
          <div className={cls.searchBlock}>
            <Input value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder='Найти ответы' />
          </div>
        </div>
